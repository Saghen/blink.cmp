...
function M.apply(edit, additional_text_edits)
  ...
  if mode == 'default' or mode == 'cmdwin' then
    ...
    if mode == 'default' and config.completion.accept.dot_repeat then M.write_to_dot_repeat(edit) end
...
--- @return lsp.TextEdit
function M.get_undo_text_edit(edit)
  return {
    range = M.get_undo_range(edit),
    newText = M.get_text_to_replace(edit),
  }
end

--- Gets the range for undoing an applied text edit
--- @param edit lsp.TextEdit
function M.get_undo_range(edit)
  edit = vim.deepcopy(edit)
...
--- @param edit lsp.TextEdit
--- @return string
function M.get_text_to_replace(edit)
  local lines = {}
  for line = edit.range.start.line, edit.range['end'].line do
    local line_text = context.get_line()
    local is_start_line = line == edit.range.start.line
    local is_end_line = line == edit.range['end'].line

    if is_start_line and is_end_line then
      table.insert(lines, line_text:sub(edit.range.start.character + 1, edit.range['end'].character))
    elseif is_start_line then
      table.insert(lines, line_text:sub(edit.range.start.character + 1))
    elseif is_end_line then
      table.insert(lines, line_text:sub(1, edit.range['end'].character))
    else
...
function M.get_from_item(item)
  ...
  local offset_encoding = text_edit.offset_encoding_from_item(item)
  text_edit = text_edit.compensate_for_cursor_movement(text_edit, item.cursor_column, context.get_cursor()[2])
  ...
  text_edit = text_edit.to_utf_8(text_edit, offset_encoding)

  text_edit.range = text_edit.clamp_range_to_bounds(text_edit.range)
...
--- @return lsp.TextEdit
function M.compensate_for_cursor_movement(edit, old_cursor_col, new_cursor_col)
  edit = vim.deepcopy(edit)

  local offset = new_cursor_col - old_cursor_col
  edit.range['end'].character = edit.range['end'].character + offset

  return edit
end

function M.offset_encoding_from_item(item)
  local client = vim.lsp.get_client_by_id(item.client_id)
  return client ~= nil and client.offset_encoding or 'utf-8'
end

function M.to_utf_8(edit, offset_encoding)
  if offset_encoding == 'utf-8' then return edit end
  edit = vim.deepcopy(edit)
  edit.range.start.character = get_line_byte_from_position(edit.range.start, offset_encoding)
  edit.range['end'].character = get_line_byte_from_position(edit.range['end'], offset_encoding)
  return edit
end
...
--- @return lsp.Range
function M.clamp_range_to_bounds(range)
...
function M.get_apply_end_position(edit, additional_text_edits)
  -- Calculate the end position of the range, ignoring the additional text edits
  local lines = vim.split(edit.newText, '\n')
  local last_line_len = #lines[#lines]
  local line_count = #lines

  local end_line = edit.range['end'].line + line_count - 1

  local end_col = last_line_len
  if line_count == 1 then end_col = end_col + edit.range.start.character end

  -- Adjust the end position based on the additional text edits
  local text_edits_before = vim.tbl_filter(
    function(additional_edit)
      return additional_edit.range.start.line < edit.range.start.line
        or additional_edit.range.start.line == edit.range.start.line
          and additional_edit.range.start.character <= edit.range.start.character
    end,
  ...
  local line_offset = 0
  local col_offset = 0
  for _, edit_before in ipairs(text_edits_before) do
    local lines_replaced = edit_before.range['end'].line - edit_before.range.start.line
    local edit_lines = vim.split(edit_before.newText, '\n')
    ...
    if edit_before.range.start.line == edit.range.start.line then
      if #edit_lines == 1 then
        local chars_replaced = edit_before.range['end'].character - edit_before.range.start.character
...
--- @param edit lsp.TextEdit
function M.write_to_dot_repeat(edit)
  ...
  local chars_to_insert = edit.newText
...
function M.move_cursor_in_dot_repeat(amount)
