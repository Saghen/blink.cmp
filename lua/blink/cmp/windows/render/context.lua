...
function context.get_from_items(items)
  ...
  for idx, item in ipairs(items) do
    ctxs[idx] = context.new(item, matched_indices[idx])
  end
  return ctxs
end
...
--- @return blink.cmp.DrawItemContext
function context.new(item, matched_indices)
  ...
  return {
    item = item,
    label = label,
    label_details = item.labelDetails and item.labelDetails.detail or '',
