...
win.update = function(self)
	if self.id ~= nil then
		...
		if self.config.relative == 'cursor' then
			local height = self:get_content_height()
			...
			if is_space_below then
				...
				vim.api.nvim_win_set_config(self.id, { relative = 'cursor', row = -height + 1, col = 0 })
			end
		-- relative to window
		...
		if self.id ~= nil then
			...
			local height = self:get_content_height()
			vim.api.nvim_win_set_height(self.id, math.min(height, self.config.max_height))
		end
	end
end

win.get_content_height = function(self)
	if not self.config.wrap then
